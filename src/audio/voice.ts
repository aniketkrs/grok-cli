 ...
 * delegated to a user-configured shell command (`voice.transcribeCommand` in
 * ~/.grok/user-settings.json); the command's stdout is inserted into the
...
export class VoiceRecorder {
  ...
  start(spec: RecorderSpec): void {
    if (this.proc) return;
    const file = join(tmpdir(), `grok-voice-${process.pid}-${Date.now()}.wav`);
