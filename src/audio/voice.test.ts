...
describe("audio.voice", () => {
  ...
  describe("findRecorder", () => {
    ...
    it("returns undefined when no recorder is installed", () => {
  ...
  describe("buildTranscribeShellCommand", () => {
    it("replaces the {file} placeholder", () => {
  ...
  describe("transcribeAudio", () => {
    it("returns the command stdout trimmed", async () => {
      const dir = mkdtempDir();
    ...
    it("rejects when the command fails", async () => {
      await expect(transcribeAudio("/tmp/a.wav", "exit 7")).rejects.toThrow("Voice transcription failed");
    });
  });

  describe("shouldDiscardTap", () => {
    it("discards holds shorter than the minimum", () => {
  ...
  describe("VoiceRecorder", () => {
    ...
    function listVoiceFiles(): string[] {
      return readdirSync(tmpdir()).filter((name) => name.startsWith("grok-voice-"));
    }
    ...
    it("starts, stops, and resolves the recorded file", async () => {
    ...
    it("second start while recording is a no-op", async () => {
      const dir = fakeBinDir({ "fake-rec": fakeRecorder });
      const recorder = new VoiceRecorder();
    ...
    it("stop without start resolves undefined", async () => {
      const recorder = new VoiceRecorder();
      expect(await recorder.stop()).toBeUndefined();
    });

    it("failed spawn clears the recording state", async () => {
      const recorder = new VoiceRecorder();
      recorder.start({ command: "grok-voice-definitely-missing", buildArgs: (file) => [file] });
