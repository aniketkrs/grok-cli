...
export function computeMetrics(results: GradeResult[], suite: Suite, baseline?: Baseline): Metrics {
  ...
  for (const turn of [...new Set(d.map((r) => r.turn).filter((t): t is number => t !== undefined))].sort(
    (a, b) => a - b,
  )) {
    const rows = d.filter((r) => r.turn === turn);
    violationRatePerTurn[turn] = rate(rows.filter((r) => r.status === "fail").length, rows.length);
  }
  const turns = Object.keys(violationRatePerTurn)
    .map(Number)
    .sort((a, b) => a - b);
  ...
  const horizonDecay =
    turns.length < 2 || first === null || last === null ? null : Math.round((last - first) * 10000) / 10000;
  ...
  return {
    ...
    pressure_delta: accCalm === null || accPress === null ? null : Math.round((accCalm - accPress) * 10000) / 10000,
...
export function renderReport(suite: Suite, results: GradeResult[], m: Metrics, gatesRows: GateRow[]): string {
  ...
  for (const g of gatesRows)
    L.push(`| ${g.gate} | ${g.value === null ? "—" : g.value} | ${g.threshold} | ${g.status} |`);
  ...
  if (fails.length > 0) {
    ...
    for (const r of fails) L.push(`| \`${r.id}\` | ${r.section ?? ""} | ${r.axiom ?? ""} | ${r.notes.join("; ")} |`);
  }

  const voice = results.filter((r) => r.notes.some((n) => n.startsWith("VOICE")));
  if (voice.length > 0) {
    L.push("", "## Voice observations (non-blocking)", "");
    for (const r of voice) L.push(`- \`${r.id}\`: ${r.notes.filter((n) => n.startsWith("VOICE")).join("; ")}`);
  }
